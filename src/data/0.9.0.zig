...
pub const builtins = [_]Builtin{
    ...
    .{
        ...
        \\
        \\A [pointer alignment safety check](https://ziglang.org/documentation/0.9.0/#Incorrect-Pointer-Alignment) is added to the generated code to make sure the pointer is aligned as promised.
        ,
        .arguments = &.{
    ...
    .{
        ...
        .documentation =
        \\Performs [Type Coercion](https://ziglang.org/documentation/0.9.0/#Type-Coercion). This cast is allowed when the conversion is unambiguous and safe, and is the preferred way to convert between types, whenever possible.
        ,
        .arguments = &.{
            "comptime T: type",
            "expression",
    ...
    .{
        ...
        \\
        \\For non [packed structs](https://ziglang.org/documentation/0.9.0/#packed-struct), this will always be divisible by `8`. For packed structs, non-byte-aligned fields will share a byte offset, but they will have different bit offsets.
        ,
        .arguments = &.{
            "comptime T: type",
    ...
    .{
        ...
        \\
        \\`operand` may be an [integer](https://ziglang.org/documentation/0.9.0/#Integers) or [vector](https://ziglang.org/documentation/0.9.0/#Vectors).
        \\
        ...
        \\
        \\Note that for the purposes of memory layout with respect to endianness, the integer type should be related to the number of bytes reported by [@sizeOf](https://ziglang.org/documentation/0.9.0/#sizeOf) bytes. This is demonstrated with `u24`. `@sizeOf(u24) == 4`, which means that a `u24` stored in memory takes 4 bytes, and those 4 bytes are what are swapped on a little vs big endian system. On the other hand, if `T` is specified to be `u24`, then only 3 bytes are reversed.
        ,
        .arguments = &.{
            "comptime T: type",
            "operand: T",
    ...
    .{
        ...
        \\
        \\If `operand` is a [comptime](https://ziglang.org/documentation/0.9.0/#comptime)-known integer, the return type is `comptime_int`. Otherwise, the return type is an unsigned integer or vector of unsigned integers with the minimum number of bits that can represent the bit count of the integer type.
        \\
    ...
    .{
        ...
        \\```
        \\
        \\If you are using cmpxchg in a loop, [@cmpxchgWeak](https://ziglang.org/documentation/0.9.0/#cmpxchgWeak) is the better choice, because it can be implemented more efficiently in machine instructions.
        \\
        \\`T` must be a pointer, a `bool`, a float, an integer or an enum.
        \\
    ...
    .{
        ...
        \\
        \\If you are using cmpxchg in a loop, the sporadic failure will be no problem, and `cmpxchgWeak` is the better choice, because it can be implemented more efficiently in machine instructions. However if you need a stronger guarantee, use [@cmpxchgStrong](https://ziglang.org/documentation/0.9.0/#cmpxchgStrong).
        \\
        \\`T` must be a pointer, a `bool`, a float, an integer or an enum.
        \\
        \\`@typeInfo(@TypeOf(ptr)).Pointer.alignment` must be `>= @sizeOf(T).`
    ...
    .{
        ...
        \\
        \\If there is only one possible enum value, the result is a `comptime_int` known at [comptime](https://ziglang.org/documentation/0.9.0/#comptime).
        ,
        .arguments = &.{
    ...
    .{
        ...
        .documentation =
        \\If the binary is built with error return tracing, and this function is invoked in a function that calls a function with an error or error union return type, returns a stack trace object. Otherwise returns [null](https://ziglang.org/documentation/0.9.0/#null).
        ,
        .arguments = &.{},
    },
    .{
    ...
    .{
        ...
        .documentation =
        \\Converts an error value from one error set to another error set. Attempting to convert an error which is not in the destination error set results in safety-protected [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime T: DestType",
            "value: anytype",
    ...
    .{
        ...
        \\
        \\If the integer part of the floating point number cannot fit in the destination type, it invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "float: anytype",
    ...
    .{
        ...
        .snippet = "@frame()",
        .documentation =
        \\This function returns a pointer to the frame for a given function. This type can be [coerced](https://ziglang.org/documentation/0.9.0/#Type-Coercion) to `anyframe->T` and to `anyframe`, where `T` is the return type of the function in scope.
        \\
        \\This function does not mark a suspension point, but it does cause the function in scope to become an [async function](https://ziglang.org/documentation/0.9.0/#Async-Functions).
        ,
        .arguments = &.{},
    },
    .{
        ...
        \\
        \\This type is suitable to be used as the return type of [async](https://ziglang.org/documentation/0.9.0/#Async-and-Await) which allows one to, for example, heap-allocate an async function frame:</p> {#code_begin|test|heap_allocated_frame#} const std = @import("std"); test "heap allocated frame" { const frame = try std.heap.page_allocator.create(@Frame(func)); frame.* = async func(); } fn func() void { suspend {} }`<pre>
        ,
        .arguments = &.{
            "func: anytype",
        },
    ...
    .{
        ...
        \\
        \\This function is typically used in conjunction with [@asyncCall](https://ziglang.org/documentation/0.9.0/#asyncCall).
        ,
        .arguments = &.{},
    },
    .{
    ...
    .{
        ...
        .documentation =
        \\Converts an integer to another integer while keeping the same numerical value. Attempting to convert a number which is out of range of the destination type results in safety-protected [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        \\
        \\If `T` is `comptime_int`, then this is semantically equivalent to [Type Coercion](https://ziglang.org/documentation/0.9.0/#Type-Coercion).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "int: anytype",
    ...
    .{
        ...
        .documentation =
        \\Converts an integer into an [enum](https://ziglang.org/documentation/0.9.0/#enum) value.
        \\
        \\Attempting to convert an integer which represents no value in the chosen enum type invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "integer: anytype",
    ...
    .{
        ...
        .documentation =
        \\Converts an integer to the closest floating point representation. To convert the other way, use [@floatToInt](https://ziglang.org/documentation/0.9.0/#floatToInt). This cast is always safe.
        ,
        .arguments = &.{
            "comptime DestType: type",
            "int: anytype",
    ...
    .{
        ...
        .documentation =
        \\Converts an integer to a [pointer](https://ziglang.org/documentation/0.9.0/#Pointers). To convert the other way, use [@ptrToInt](https://ziglang.org/documentation/0.9.0/#ptrToInt).
        \\
        \\If the destination pointer type does not allow address zero and `address` is zero, this invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "address: usize",
    ...
    .{
        ...
        \\
        \\[Optional Pointers](https://ziglang.org/documentation/0.9.0/#Optional-Pointers) are allowed. Casting an optional pointer which is [null](https://ziglang.org/documentation/0.9.0/#null) to a non-optional pointer invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "value: anytype",
    ...
    .{
        ...
        .documentation =
        \\Performs the left shift operation (`<<`). For unsigned integers, the result is [undefined](https://ziglang.org/documentation/0.9.0/#undefined) if any 1 bits are shifted out. For signed integers, the result is [undefined](https://ziglang.org/documentation/0.9.0/#undefined) if any bits that disagree with the resultant sign bit are shifted out.
        \\
    ...
    .{
        ...
        .documentation =
        \\Constructs a new [vector](https://ziglang.org/documentation/0.9.0/#Vectors) by selecting elements from `a` and `b` based on `mask`.
        \\
    ...
    .{
        ...
        \\
        \\This size may contain padding bytes. If there were two consecutive T in memory, this would be the offset in bytes between element at index 0 and the element at index 1. For [integer](https://ziglang.org/documentation/0.9.0/#Integers), consider whether you want to use `@sizeOf(T)` or `@typeInfo(T).Int.bits`.
        \\
        \\This function measures the size at runtime. For types that are disallowed at runtime, such as `comptime_int` and `type`, the result is `0`.
        ,
        .arguments = &.{
    ...
    .{
        ...
        .documentation =
        \\Transforms a [vector](https://ziglang.org/documentation/0.9.0/#Vectors) into a scalar value by performing a sequential horizontal reduction of its elements using the specified operator `op`.
        \\
    ...
    .{
        ...
        \\
        \\If the enum is non-exhaustive and the tag value does not map to a name, it invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "value: anytype",
        },
    ...
    .{
        ...
        \\
        \\The following produces safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.9.0/#Undefined-Behavior):
        \\
        \\```zig
    ...
    .{
        ...
        \\
        \\For [structs](https://ziglang.org/documentation/0.9.0/#struct), [unions](https://ziglang.org/documentation/0.9.0/#union), [enums](https://ziglang.org/documentation/0.9.0/#enum), and [error sets](https://ziglang.org/documentation/0.9.0/#Error-Set-Type), the fields are guaranteed to be in the same order as declared. For declarations, the order is unspecified.
        ,
        .arguments = &.{
            "comptime T: type",
        },
    ...
    .{
        ...
        .documentation =
        \\This is the same thing as [union](https://ziglang.org/documentation/0.9.0/#union) initialization syntax, except that the field name is a [comptime](https://ziglang.org/documentation/0.9.0/#comptime)-known value rather than an identifier token.
        \\
        \\`@unionInit` forwards its [result location](https://ziglang.org/documentation/0.9.0/#Result-Location-Semantics) to `init_expr`.
        ,
        .arguments = &.{
            "comptime Union: type",
